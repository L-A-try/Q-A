...
public class SkelOptor implements Optimisable {
    ...
    public DataFile[] getBestFile( String[] lfns, 
		...
		for(i=0;i<lfns.length;i++) {
			...
			int masite=2;
			GridSite gsite = gc.findGridSiteByID(masite);//主站点 文件名 lfns【i】
			StorageElement sss=gsite.getSE();
			DataFile dmFile = sss.requestFile(lfns[i]);//主文件
			Date mchangetime=dmFile.getLastChangeTime();//主文件时间戳
