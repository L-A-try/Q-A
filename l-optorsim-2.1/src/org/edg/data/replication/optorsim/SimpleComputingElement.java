	...
	**/
    ...
    public SimpleComputingElement( GridSite site,  int workerNodes, float capacity) {
		...
		**/
		...
		double wirtePercent=0.2;
		_writenumjobs = (int)(_numjobs*wirtePercent);
     ...
     */
    public void run() {
    	System.out.println("--------SimpleComputingElement---run--------------");

        // Boost our priority
        Thread.currentThread().setPriority(Thread.MAX_PRIORITY);
        Double execTime;
		OptorSimParameters params = OptorSimParameters.getInstance();
        GridContainer gcc = GridContainer.getInstance();
        int masite=2;
        GridSite gsite = gcc.findGridSiteByID(masite);//主站点 文件名 lfns【i】
        StorageElement sss=gsite.getSE();
    	_runnable = true;
		...
		for( GridJob job=null; job != null || _imAlive; ) {
		    ...
		    for( String lfn = accessPatternGenerator.getNextFile();
			 ...
			 lfn = accessPatternGenerator.getNextFile()) {
				...
				if( _site != fileSite) {
					System.out.println("SimpleCompuyingElement 不是一个站点 如果文件是远程的，则模拟远程IO，取消固定并转到下一个文件。");
...
//						_site.getCloseSE().accessFile(files[0]);
						/**------------------------------------
						     ...
						     * @return 写读操作分开
						**/
						proJob(_site.getCloseSE(),files[0]);
					}
					if(_workerNodes != 0) {
...
//					fileSE.accessFile(files[0]);
	...
	/**------------------------------------
	     ...
	     * @return 作业类型设置
	**/
	protected void proJob(StorageElement se,DataFile file){
		Random random = new Random();
		if( _writenumjobs>0) {//是不是写
			if(random.nextFloat() < 0.5){//写
				se.writeFile(file);
				_writenumjobs--;
			}
			else//读
