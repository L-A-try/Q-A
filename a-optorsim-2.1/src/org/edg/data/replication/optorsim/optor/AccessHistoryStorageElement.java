...
public class AccessHistoryStorageElement extends SimpleStorageElement {
	...
	*/
	...
	/**------------------------------------
	     ...
	     * @return 读文件
	**/
	public synchronized void readFile(DataFile file) {
		System.out.println("--------AccessHistoryStorageElement---readFile--------------");
	...
	/**------------------------------------
	     ...
	     * @return 写文件
	**/
	public synchronized void writeFile(DataFile file) {
		System.out.println("--------AccessHistoryStorageElement---writeFile--------------");
		_accessHistory.put(_time.getDate(),  file);
		String lfn = file.lfn();
		...
		for(int j=0;j<dataFiles.length;j++) {//对每个文件更新
			dataFiles[j].write(); // pin the file
			rm.registerEntry(file);
		}
    ...
    public synchronized void accessFile(DataFile file) {
        System.out.println("--------AccessHistoryStorageElement---accessFile--------------");
        _accessHistory.put(_time.getDate(),  file);
        // TODO: get all file history out of DataFile.
        file.read();
        //必须主副本更新一下
