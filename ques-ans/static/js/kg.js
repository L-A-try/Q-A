initKG = function (data, config, container) {
    ...
    if (!config.nodeColor || config.nodeColor === "")
        var defaultNodeColor = [
            ...
            //灰色
            { fill: "#e2dbbe", stroke: "#ffee93", text: "#ffc09f"},
    ...
    var circle = svg.append("g")
        ...
        .style("fill", function (node) {
            // node.type默认是default
            var my_color_index = Math.floor(Math.random()*defaultNodeColor.length);
            return defaultNodeColor[my_color_index].fill;
        })
        .style('stroke', function (node) {
            //config:边框色
            var my_color_index = Math.floor(Math.random()*defaultNodeColor.length);
            return defaultNodeColor[my_color_index].stroke;
        })
        .attr("r", 30)
        ...
        .on("mouseover", function (d, i) {
            ...
            var tspan_tag = document.getElementsByTagName('tspan')[i]
            var real_value = tspan_tag.getAttribute('my_value')
            // tspan_tag.setAttribute('my_value', tspan_tag.innerHTML)
            // tspan_tag.innerHTML = real_value

            $(".card-title").html(real_value);
    ...
    var text = svg.append("g").selectAll("text")
        ...
        .attr('x', function (d) {
            //如果小于8个字符，不换行
            var max_length = 5;
            if (d.name.length <= max_length) {
                d3.select(this).append('tspan')
                    .attr('x', 0)
                    .attr('y', 2)
                    .attr("my_value", d.name)
                    .text(function () { return d.name; });
            }
            else if (d.name.length > max_length) {//大于16个字符时，将14个字后的内容显示为。。。
